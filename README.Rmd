...
Stored statements can be retreived from Learning Locker by using the `api_request()` method. **Note**: This requires a connection to be established by the R-handler to work; JS implementations can interface with the api through [HTTP GET](https://developer.mozilla.org/en-US/docs/Web/HTTP/Methods/GET) / [XMLHttpRequest](https://developer.mozilla.org/en-US/docs/Web/API/XMLHttpRequest/Using_XMLHttpRequest) instead.
